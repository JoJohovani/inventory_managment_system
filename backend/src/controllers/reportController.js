...
const getTopSellingProducts = async (req, res, next) => {
  try {
    ...
    const topProducts = await saleService.getTopSellingProducts(startDate, endDate, Number.parseInt(limit))
...
const getDashboardStats = async (req, res, next) => {
  try {
    ...
    const dailyPurchases = await purchaseService.getPurchaseReport(startOfDay, endOfDay)

    // Get monthly stats
    const monthlySales = await saleService.getSalesReport(startOfMonth, endOfMonth)
    const monthlyPurchases = await purchaseService.getPurchaseReport(startOfMonth, endOfMonth)
