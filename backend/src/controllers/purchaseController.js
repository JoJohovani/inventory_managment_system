...
const getAllPurchases = async (req, res, next) => {
  try {
    const { page = 1, limit = 10, startDate, endDate, supplierId, status } = req.query
...
const updatePurchaseStatus = async (req, res, next) => {
  try {
    ...
    const purchase = await purchaseService.updatePurchaseStatus(Number.parseInt(id), status)
