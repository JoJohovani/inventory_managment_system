...
class UserService {
  ...
  async getUserProfile(userId) {
    const user = await userRepository.findUnique({ id: userId }, { role: true })
    if (!user) {
  ...
  async getUserById(userId) {
    const user = await userRepository.findUnique({ id: userId, isDeleted: false }, { role: true })
    if (!user) {
      throw new Error("User not found")
    }
  ...
  async deleteUser(userId) {
    const user = await userRepository.findUnique({ id: userId, isDeleted: false })
    if (!user) {
      throw new Error("User not found")
    }
