...
class PurchaseService {
  async createPurchase(purchaseData, userId) {
    ...
    const completePurchase = await purchaseRepository.findPurchaseWithDetails(
      purchase.id
    );
  ...
  async getAllPurchases(page = 1, limit = 10, filters = {}) {
    return await purchaseRepository.findPurchasesWithDetails(
      page,
      limit,
      filters
    );
  ...
  async getPurchaseById(purchaseId) {
    const purchase = await purchaseRepository.findPurchaseWithDetails(
      purchaseId
    );
    if (!purchase) {
    ...
    if (oldStatus !== "received" && status === "received") {
      for (const productPurchase of purchase.productPurchases) {
        await productRepository.incrementStock(
          ...
          productPurchase.purchase_quantity
        );

        // Update product cost
        await productRepository.update(
          { id: productPurchase.productId },
          { cost: productPurchase.purchase_price }
        );
      }
    }
  ...
  async getPurchasesByUser(userId, page = 1, limit = 10) {
    return await purchaseRepository.findPurchasesWithDetails(page, limit, {
      userId,
    });
  }
